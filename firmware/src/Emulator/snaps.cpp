...
int Load_SNA(ZXSpectrum *speccy, const char *filename)
{
  ...
  { // es un 48Kb
    fread(speccy->mem.p + 16384, 0x4000 * 3, 1, fp);
  ...
  { // es un 16Kb UNSUPPORTED
    fread(speccy->mem.p + 16384, 0x4000, 1, fp);
    speccy->z80Regs->PC.B.l = speccy->z80_peek(speccy->z80Regs->SP.W);
    speccy->z80Regs->SP.W++;
    speccy->z80Regs->PC.B.h = speccy->z80_peek(speccy->z80Regs->SP.W);
    speccy->z80Regs->SP.W++;
  ...
  { // es un 128Kb FIXME hay SNA de 16Kb
    ...
    fread(speccy->mem.p + (5 * 0x4000), 0x4000, 1, fp);
    fread(speccy->mem.p + (2 * 0x4000), 0x4000, 1, fp);
    fread(speccy->mem.p + (page * 0x4000), 0x4000, 1, fp);
    // resto de paginas.
    fseek(fp, 49183, SEEK_SET);
    if (page != 0)
      fread(speccy->mem.p + (0 * 0x4000), 0x4000, 1, fp);
    if (page != 1)
      fread(speccy->mem.p + (1 * 0x4000), 0x4000, 1, fp);
    if (page != 3)
      fread(speccy->mem.p + (3 * 0x4000), 0x4000, 1, fp);
    if (page != 4)
      fread(speccy->mem.p + (4 * 0x4000), 0x4000, 1, fp);
    if (page != 6)
      fread(speccy->mem.p + (6 * 0x4000), 0x4000, 1, fp);
    if (page != 7)
      fread(speccy->mem.p + (7 * 0x4000), 0x4000, 1, fp);
  }
  fclose(fp);
  return (0);
}
