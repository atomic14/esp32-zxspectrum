...
static constexpr uint8_t FRAME_BYTE = 0x7E;
...
private:
  ...
  enum class State
  {
    ...
    // we've started a packet and are waiting for the command type
    READING_COMMAND_TYPE,
    // we've read the command type and are waiting for the command length
    READING_COMMAND_LENGTH,
    // we've read the command length and are waiting for the data
    READING_DATA,
  ...
  using CommandStartCallback = std::function<void(uint32_t)>;
  using CommandDataCallback = std::function<void(const uint8_t *, uint16_t)>;
  using CommandFinishedCallback = std::function<void(bool)>;
  ...
  std::map<uint8_t, CommandStartCallback> commandStartHandlers;
  std::map<uint8_t, CommandDataCallback> commandDataHandlers;
  std::map<uint8_t, CommandFinishedCallback> commandFinishedHandlers;
...
public:
  ...
  void registerCommandHandler(uint8_t type, CommandStartCallback startCallback, CommandDataCallback dataCallback, CommandFinishedCallback finishedCallback)
  {
    if (startCallback)
    {
      commandStartHandlers[type] = startCallback;
    }
    else
    {
      commandStartHandlers.erase(type);
    }
    if (dataCallback)
    {
      commandDataHandlers[type] = dataCallback;
    }
    else
    {
      commandDataHandlers.erase(type);
    }
    if (finishedCallback)
    {
      commandFinishedHandlers[type] = finishedCallback;
    }
    else
    {
      commandFinishedHandlers.erase(type);
    }
  }
  ...
  State processExpectingFrameByte(uint8_t data)
  {
    ...
    if (commandFinishedHandlers.find(commandType) != commandFinishedHandlers.end())
    {
      commandFinishedHandlers[commandType](isValid);
    }
    return State::WAITING_FOR_START_BYTE;
  }

  State
  processReadingCommandType(uint8_t data)
  {
    commandType = data;
    lengthByteCount = 0;
    commandLength = 0;
  ...
  State processReadingCommandLength(uint8_t data)
  {
    ...
    if (commandStartHandlers.find(commandType) != commandStartHandlers.end())
    {
      commandStartHandlers[commandType](commandLength);
    }
    totalRead = 0;
    bufferPosition = 0;
  ...
  State processReadingData(uint8_t data)
  {
    if (totalRead < commandLength)
    {
      ...
      if (bufferPosition > PACKET_DATA_BUFFER_SIZE || totalRead == commandLength)
      {
        if (commandDataHandlers.find(commandType) != commandDataHandlers.end())
        {
          commandDataHandlers[commandType](dataBuffer, bufferPosition);
        }
        bufferPosition = 0;
      }
    }
  ...
  void loop()
  {
    ...
    else
    {
      ...
      if (data == FRAME_BYTE)
      {
        if (commandFinishedHandlers.find(commandType) != commandFinishedHandlers.end())
        {
          commandFinishedHandlers[commandType](false);
        }
      ...
      case State::READING_COMMAND_TYPE:
        state = processReadingCommandType(data);
        break;
      case State::READING_COMMAND_LENGTH:
        state = processReadingCommandLength(data);
        break;
   ...
   */
  void sendPacket(uint8_t type, const uint8_t *data, uint32_t length)
  {
    transport.write(FRAME_BYTE);
    uint32_t crc = write(type);
